...
/* int methodOfSort;*//*
    ...
    private void downloadData(int methodOfSort, int page) {
        URL url = NetworkUtils.buildURL(methodOfSort, page);//formiruyem Url
    ...
    public void onLoadFinished(@NonNull Loader<JSONObject> loader, JSONObject jsonObject) {
        ArrayList<Movie> movies = JsonUtils.getMoviesFromJSON(jsonObject);
        if (movies != null && !movies.isEmpty()) { // vse zagrujenniy filmi  dobavlayetsya na bd  a takje dobavlaetsya v adapter
            if (page == 1) {
                viewModel.deleteAllMovies();//mi ochsteim predudidushie danniy
                movieAdapter.clear();
            }
            for (Movie movie : movies) {  // vstavim danniy
                viewModel.insertMovie(movie);
            }
            movieAdapter.addMovies(movies);   // add new films
            page++; // danniy zagruilis mi budev uvelechit nomer stranitsi na yedinitsi page ++
        }
        isLoading = false;
